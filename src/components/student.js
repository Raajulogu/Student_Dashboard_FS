...
function Students({student,setstudent}){
    async function deleteStudent(Id){
        let response = await fetch(`https://644b33c017e2663b9deab958.mockapi.io/users/${Id}`,{
          method: "DELETE", 
        });
        let data =await response.json();
        if(data){
            let remainingdata = student.filter((prod,index)=>prod.id!==Id);
